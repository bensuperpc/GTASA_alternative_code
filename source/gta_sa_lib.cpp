 ...
 */
constexpr std::array<uint32_t, 256> gta::precompute_crc()
{
  std::array<uint32_t, 256> _crc32_lookup = {0};
  // compute each power of two (all numbers with exactly one bit set)
  uint32_t crc = _crc32_lookup[0x80] = polynomial;
  for (std::uint32_t next = 0x40; next != 0; next >>= 1) {
    crc = (crc >> 1) ^ ((crc & 1) * polynomial);
    _crc32_lookup[next] = crc;
  }

  for (std::uint32_t power_of_two = 2; power_of_two <= 0x80; power_of_two <<= 1)
  {
    uint32_t crc_extra_bit = _crc32_lookup[power_of_two];
    for (std::uint32_t i = 1; i < power_of_two; i++) {
      _crc32_lookup[i + power_of_two] = _crc32_lookup[i] ^ crc_extra_bit;
    }
  }
  return static_cast<const std::array<uint32_t, 256>>(_crc32_lookup);
}

constexpr std::array<uint32_t, 256> crc32_lookup = gta::precompute_crc();
