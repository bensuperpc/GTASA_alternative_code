...
auto gta::precompute_crc() -> std::array<uint32_t, 256>
{
  ...
  uint32_t crc = crc32_lookup_table[0x80] = polynomial;
  for (std::uint32_t next = 0x40; next != 0; next >>= 1) {
    crc = (crc >> 1) ^ ((crc & 1) * polynomial);
    crc32_lookup_table[next] = crc;
  }
  ...
  for (std::uint32_t power_of_two = 2; power_of_two <= 0x80; power_of_two <<= 1)
  {
    ...
    for (std::uint32_t i = 1; i < power_of_two; i++) {
      crc32_lookup_table[i + power_of_two] = crc32_lookup[i] ^ crc_extra_bit;
    }
  }
  return crc32_lookup_table;
}